...
import { Search, Plus, ArrowUpRight, CheckCircle, Clock, ArrowLeft, Save, Trash2, Edit2, X, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function ContasReceber() {
  const [contas, setContas] = useState<Receber[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [view, setView] = useState<'list' | 'form'>('list');
  const [editingItem, setEditingItem] = useState<Receber | null>(null);
  ...
  async function fetchContas() {
    ...
    try {
      const { data, error } = await supabase.from('contas_receber').select('*').order('created_at', { ascending: false });
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingItem) {
        const { error } = await supabase.from('contas_receber').update(contaData).eq('id', editingItem.id);
        if (error) throw error;
        setContas(contas.map(c => c.id === editingItem.id ? { ...c, ...contaData } : c));
      } else {
        const { data, error } = await supabase.from('contas_receber').insert([contaData]).select();
        if (error) throw error;
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase.from('contas_receber').delete().eq('id', deleteModalId);
      if (error) throw error;
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-6 w-full">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={saving} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition-colors flex items-center gap-2 shadow-sm disabled:opacity-50">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="space-y-2">
               ...
               <input required name="cliente" type="text" defaultValue={editingItem?.cliente} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
             </div>
             <div className="space-y-2">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
        ...
        <div className="overflow-x-auto">
          {loading ? (
             ...
             <div className="p-8 text-center text-slate-500">Nenhum título encontrado.</div>
          ) : (

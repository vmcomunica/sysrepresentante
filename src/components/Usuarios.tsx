        ...
        <form onSubmit={handleSave} className="flex flex-col gap-6 w-full">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-800">{editingItem ? 'Editar Usuário' : 'Novo Usuário'}</h1>
                <p className="text-sm text-slate-500 mt-1">Configure o acesso do funcionário.</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="space-y-2">
               ...
               <input required name="nome" type="text" defaultValue={editingItem?.nome} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
             </div>
             <div className="space-y-2">
               <label className="text-sm font-medium text-slate-700">E-mail de Acesso *</label>
               <input required name="email" type="email" defaultValue={editingItem?.email} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
             </div>
             <div className="space-y-2">
               <label className="text-sm font-medium text-slate-700">Senha {editingItem ? '(Deixe em branco para não alterar)' : '*'}</label>
               <input name="senha" type="password" required={!editingItem} minLength={6} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
             </div>
             <div className="space-y-2">
               <label className="text-sm font-medium text-slate-700">Confirmar Senha</label>
               <input name="confirm_senha" type="password" required={!editingItem} minLength={6} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
             </div>
             <div className="space-y-2">
               <label className="text-sm font-medium text-slate-700">Perfil de Acesso (RBAC) *</label>
               <select name="perfil" defaultValue={editingItem?.perfil || 'vendedor'} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    <div className="flex flex-col gap-6 w-full pb-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Gerencie a equipe e controle os acessos ao sistema.</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col">
        <div className="p-4 border-b border-slate-100 flex items-center">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Buscar por nome ou e-mail..." className="pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm w-full focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
        </div>
      ...
      {deleteModalId && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-slate-600 mb-6">Atenção! Ao excluir este usuário, ele perderá acesso ao sistema imediatamente.</p>
